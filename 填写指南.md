...
双击 `index.html` 可以直接用浏览器打开。如果想更接近线上环境（相对路径、图片懒加载等行为一致），在项目目录执行任意一个：

```bash
# Node（本机已装）
node "C:\Users\huang\AppData\Local\Temp\homepage-preview-server.mjs" . 8080
...
1. 在 GitHub 上建一个仓库。**如果想让网址是 `https://用户名.github.io`，仓库名必须叫 `用户名.github.io`**；叫别的名字的话网址会是 `https://用户名.github.io/仓库名/`。
2. 本地初始化并推送：
...
> 建议加一个 `.nojekyll` 空文件（本仓库已包含），否则 GitHub Pages 的 Jekyll 处理可能会忽略某些以下划线开头的文件。
